...
const createTicket = async (req, res) => {
  try {
    ...
    const assigneeUser = await User.findOne({ email: assigneeEmail, organisationNames:organisation });
    if (!assigneeUser) {
...
//     const assigneeUser = await User.findOne({ email: assigneeEmail, organisation });
//     if (!assigneeUser) {
//       return res.status(400).json({ message: 'Assignee not found in your organization' });
//     }
